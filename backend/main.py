...
app = FastAPI(title="Scaler Persona Bot Backend")
...
async def chat_with_persona(request: ChatRequest):
    ...
    if not model_name:
        raise HTTPException(status_code=500, detail="HF_MODEL not configured.")
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="AI provider unreachable.") from exc
